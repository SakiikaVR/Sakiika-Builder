...
- 🎬 **スプラッシュの無効化** — 半透明テーマへの差し替えで Android 12+ のシステムスプラッシュごと非表示にできます (minSdk 28 以上、または画面の向きを固定しないとき。それ以外は実質見えないレベルまで短縮)
...
- 署名鍵 (`sakiika-key.pem`) は自己署名の EC P-256 鍵で、0.2.0 からは**証明書も同じファイルに保存してビルド間で再利用**します。**Android は同じ証明書でないと上書き更新を受け付けないため、このファイルは必ず保管してください** (0.1.0 の鍵ファイルは次のビルドで自動的に証明書が追記されます。ただし 0.1.0 でビルドした APK の証明書は復元できないため、その端末では一度アンインストールが必要です)
