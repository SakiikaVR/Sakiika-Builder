...
public sealed partial class MainWindow : Window
{
    ...
    /// <summary>
    /// Loads the header image and the window/taskbar icon from files next to the
    /// executable, rather than as ms-appx resources: this app is unpackaged and
    /// its PRI is generated by hand, so file paths are the dependable route.
    /// </summary>
    private void ApplyBranding()
    {
        ...
        try
        {
            var png = Path.Combine(assets, "icon.png");
            if (File.Exists(png))
            {
                BrandImage.Source = new BitmapImage(new Uri(png));
            }
        }
        catch (Exception e)
        {
            Program.Log("branding", e, "ヘッダー画像を読めませんでした");
        }
        try
        {
        ...
        {
            Program.Log("branding", e, "ウィンドウアイコンを設定できませんでした");
        }
    }
