...
抑制はあくまで「起動時のテーマ」で決まるため、MainActivity は起動直後に
`setTranslucent(false)` (API 30+) でウィンドウを不透明へ戻します。また
`Theme.Translucent` は `windowDrawsSystemBarBackgrounds` を持たないため、
`FLAG_DRAWS_SYSTEM_BAR_BACKGROUNDS` を実行時に付与してから
ステータスバー / ナビゲーションバーをライト・ダークの設定色で塗ります
(0.2.0 ではこれが抜けており、バーの色がテーマに追従しませんでした)。
